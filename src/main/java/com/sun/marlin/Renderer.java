...
// TODO: reimplement block flags for FX
    static final boolean ENABLE_BLOCK_FLAGS = false && MarlinProperties.isUseTileFlags();
...
//                System.out.println("quadTo: curve clipped: X");
    ...
    private void _endRendering(final int ymin, final int ymax, AlphaConsumer ac) {
        if (DISABLE_RENDER) {
            return;
        }
...
// TODO: reimplement block flags for FX
        boolean useBlkFlags = false && this.prevUseBlkFlags;
...
//                            x1 = (curx < bboxx1) ? curx : bboxx1;
            ...
            if ((y & _SUBPIXEL_MASK_Y) == _SUBPIXEL_MASK_Y) {
                ...
                if (maxX >= minX) {
                    ...
                    ac.setAndClearRelativeAlphas(_alpha, lastY, minX, maxX + 1);
//                    copyAARow(_alpha, lastY, minX, maxX + 1, useBlkFlags);
                    ...
                    ac.setAndClearRelativeAlphas(_alpha, lastY, 1, 0); // 1 > 0 means clear row
//                    _cache.clearAARow(lastY);
        ...
        if (maxX >= minX) {
            ...
            ac.setAndClearRelativeAlphas(_alpha, y, minX, maxX + 1);
//            copyAARow(_alpha, y, minX, maxX + 1, useBlkFlags);
        } else if (y != lastY) {
            ac.setAndClearRelativeAlphas(_alpha, y, 1, 0); // 1 > 0 means clear row
//            _cache.clearAARow(y);
        }
...
//            enableBlkFlags = this.cache.useRLE;
            ...
            if (enableBlkFlags) {
                ...
                final int nxTiles = ((pmaxX - pminX) >> TILE_W_LG) + 2;
    ...
    void endRendering(final int pminY, AlphaConsumer ac) {
        if (DO_MONITORS) {
...
//            cache.resetTileLine(pminY);
            ...
            _endRendering(fixed_spminY, spmaxY, ac);
        }
        if (DO_MONITORS) {
